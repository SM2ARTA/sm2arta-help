...
set "LOG=%REPO%\publish.log"
set "INJLOG=%REPO%\inject-guard.log"
...
rem === 1) run injector BEFORE staging files (silent; log to file) ===
if exist "%INJECT%" (
  "%PWSH%" -NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INJECT%" >> "%INJLOG%" 2>&1
) else (
  echo [WARN] %date% %time% Injector not found: %INJECT%>> "%INJLOG%"
)
...
rem === 4) commit & push (silent; log to file) ===
%GIT% add -A >> "%LOG%" 2>&1
